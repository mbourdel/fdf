...
/*   Updated: 2014/12/06 19:17:12 by mbourdel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_draw_line(t_ls2d pt2d, const t_env *env, int i, int color)
{
	...
	{
		if (color == 1)
			mlx_pixel_put(env->mlx, env->win, pt2d[i - 1].x, pt2d[i - 1].y, ((WHITE - ((pt2d[i - 1].y) * 1000))));
		else
			mlx_pixel_put(env->mlx, env->win, pt2d[i - 1].x, pt2d[i - 1].y, ((WHITE - ((pt2d[i].y) * 1000))));
...
static void		ft_draw_pt2d(t_ls2d pt2d, const t_env *env)
{
	...
	while (pt2d[i].x != 0)
	{
		if ((pt2d[i].y != pt2d[i - j].y) && (pt2d[i].x != XBEGIN))
		{
			mlx_pixel_put(env->mlx, env->win, pt2d[i].x, pt2d[i].y, (WHITE - ((pt2d[i - j].y) * 1000)));
...
//			mlx_pixel_put(env->mlx, env->win, pt2d[i].x + 1, pt2d[i].y + 1, (WHITE - ((pt2d[i - j].y) * 1000)));
		...
		{
			j = 1;
			mlx_pixel_put(env->mlx, env->win, pt2d[i].x, pt2d[i].y, (WHITE - ((pt2d[i].y) * 1000)));
